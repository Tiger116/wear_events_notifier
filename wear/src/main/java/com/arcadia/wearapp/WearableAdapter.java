...
public class WearableAdapter extends WearableListView.Adapter {
    ...
    private Comparator comparator = new Comparator<Event>() {
        @Override
    ...
    public void onBindViewHolder(WearableListView.ViewHolder holder,
        ...
        if (event.getStartDate() != null) {
            itemHolder.secondaryTextView.setText(new SimpleDateFormat("EEE d MMM h:mm a", Locale.ROOT).format(event.getStartDate()));
            if (event.getEndDate() != null && event.getEndDate().after(event.getStartDate()))
                itemHolder.secondaryTextView.append(String.format(" - %s", new SimpleDateFormat("EEE d MMM h:mm a", Locale.ROOT).format(event.getEndDate())));
        }
